...
public class GameAPI : MonoBehaviour
{
  private string baseUrl = "http://localhost:4000/api";
    ...
    public IEnumerator RegisterPlayer(string playerName, string passoword)
    {
        var requestData = new { name = playerName, password = passoword };
        ...
        using (UnityWebRequest request = new UnityWebRequest($"{baseUrl}/register", "POST"))
        {
            ...
            {
                Debug.Log($"Player registered successfully: {request.downloadHandler.text}");
            }
        }
    ...
    public IEnumerator LoginPlayer(string playerName, string password, Action<PlayerModel> onSuccess)
    {
        ...
        using (UnityWebRequest request = new UnityWebRequest($"{baseUrl}/login", "POST"))
        {
            ...
            {
                Debug.LogError($"Error logging in player: {request.result}");
            }
            else
            {
